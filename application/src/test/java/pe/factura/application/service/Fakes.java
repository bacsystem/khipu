...
final class Fakes {
    ...
    static final class Establecimientos implements pe.factura.application.port.out.EstablecimientoRepository {
        ...
        public java.util.Optional<pe.factura.application.port.out.EstablecimientoRepository.Asignacion> buscarAsignacionDeSerie(UUID t, TipoDocumento tipo, String serie) {
            return series.buscar(t, tipo, serie).filter(s -> !s.enDomicilioFiscal())
                    .map(s -> new pe.factura.application.port.out.EstablecimientoRepository.Asignacion(s.establecimiento(), datos.get(t + s.establecimiento())));
        }
    }
