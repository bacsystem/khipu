...
/**
 * Adquirente. Se valida al emitir ({@link #exigirValidoParaFactura(String, boolean)}), no al rehidratar: los comprobantes ya
...
public record Receptor(String tipoDoc, String numDoc, String razonSocial, String direccion, String pais) {
    ...
    void exigirValidoParaFactura() { exigirValidoParaFactura("0101", false); }

    /**
     * Venta interna: RUC con dígito verificador (2017). Exportación (0200–0208): documento del catálogo 06 con su formato
     * (2800–2802), país del receptor, y sin RUC en 0200/0201/0204 salvo con la leyenda 2008 (2800). Razón social de 3 a 1500
     * caracteres sin saltos de línea (2021/2022) en ambos casos.
     */
    void exigirValidoParaFactura(String tipoOperacion, boolean conLeyenda2008) {
        if (Exportacion.es(tipoOperacion)) exigirValidoParaExportacion(tipoOperacion, conLeyenda2008);
        else {
    ...
    private void exigirValidoParaExportacion(String tipoOperacion, boolean conLeyenda2008) {
        ...
        if ("6".equals(tipoDoc)) {
            if (Exportacion.SIN_RUC.contains(tipoOperacion) && !conLeyenda2008)
                throw new DomainException("RECEPTOR_INVALIDO", "2800 - La exportación " + tipoOperacion + " no admite un receptor con RUC (tipo_doc 6) salvo con la leyenda 2008");
            Ruc.exigirValido(numDoc, "RECEPTOR_INVALIDO", "2017 - Receptor");
