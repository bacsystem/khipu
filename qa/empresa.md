...
| 🔧 parcial | Paridad del mock (5 huecos). **Corregido**: `padron_tasa_especial_igv` ya se persiste (campo de dinero: decide 10.5 % vs 18 %). **Abiertos**: acepta cualquier «certificado» sin parsearlo, acepta SOL en blanco, solo valida el ubigeo de los datos fiscales, y deja registrar dos empresas con el mismo RUC | `src/mocks/handlers.ts` | ver pendientes |
...
## Verificación de los arreglos (segunda ronda, PR #162)

| Mutación | Resultado |
|---|---|
| Usuario SOL sin `strip` · sin chequeo de espacios internos | mueren |
| Sin handler de archivo grande | muere |
| Guarda del distrito inerte · sin mirar la urbanización | mueren |
| No-op Java y no-op portal (controles) | sobreviven |

Los menores de mensajes y del 401 de `CREDENCIALES_INVALIDAS` también quedaron cerrados: el código vuelve a significar solo «login fallido», y se agregaron las nueve claves que faltaban en `es.json`.

## Verificación de los arreglos (primera ronda, PR #160)
